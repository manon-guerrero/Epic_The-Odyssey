  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (container) {
        ...
        const boundedIndex = Math.min(Math.max(0, currentIndex), sagas.length - 1);
    ...
    <div className="app-layout">
      ...
      <div className="container">
        {sagas.map((saga) => (
          <div 
            key={saga.id} 
            className={`saga-card ${flippedCardId === saga.id ? 'flipped' : ''}`}
          >
            <div 
              className={`saga-card-inner ${flippedCardId === saga.id ? 'flipped' : ''}`}
      ...
      <div className={`info-panel ${!showInfo ? 'hidden' : ''}`}>
        <div className="info-content">
          <h2>{sagas.find(saga => saga.id === activeSagaId)?.title}</h2>
